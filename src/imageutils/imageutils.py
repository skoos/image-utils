...
def img_to_array(img, data_format='channels_last', data_type=np.float32):
    ...
    if data_format not in {'channels_first', 'channels_last'}:
        LOGGER.WARN('Unknown/missing data format in img_to_array function: ' + data_format)
        return None
    ...
    if len(img_data.shape) == 3:
        ...
        LOGGER.WARN('Unsupported image shape in img_to_array function: ' + str(img_data.shape))
        return None
...
def resize_img(img, target_size):
    ...
    if target_size is None \
       ...
       or len(target_size) != 2:
        LOGGER.WARN('Unexpected size dimension for resize_img function: ' + str(target_size))
        return None
...
def crop_img(img, target_crop):
    ...
    if target_crop is None \
       ...
       or len(target_crop) != 4:
        LOGGER.WARN('Unexpected crop dimension for crop_img function: ' + str(target_crop))
        return None
...
def load_img(path, target_size=None):
    ...
    if Image is None:
        LOGGER.WARN('Could not import PIL.Image. The use of `load_img` requires PIL.')
        return None
        #raise ImportError('Could not import PIL.Image. '
        #                  'The use of `load_img` requires PIL.')
    if not os.path.exists(path):
        LOGGER.WARN('Directory not found ing load_img function: ' + path)
        return None
...
def save_img(img, output_path, quality=95):
    ...
    if not output_path or not os.path.exists(os.path.dirname(output_path)):
        LOGGER.WARN('Directory not found in save_img function: ' + output_path)
...
def load_img_from_md5(image_md5, url='https://octopus.heuritech.com/get?md5='):
    ...
    try:
        ...
        LOGGER.WARN('Error in load_img_from_md5: ' + str(error))
        return None
    return img
...
def get_md5_from_imagepath(image_path):
    ...
    if image_path is None or not os.path.exists(os.path.dirname(image_path)):
        LOGGER.WARN('Directory not found in get_md5_from_imagepath function: ' + image_path)
        return None
